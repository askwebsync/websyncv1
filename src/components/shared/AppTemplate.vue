<template>
  <div class="template-background font">
    <div class="container mx-auto">
      <div
        class="
          flex flex-col
          py-12
          gap-y-2
          md:grid md:grid-cols-2 md:gap-10 md:py-36
        "
      >
        <div class="border-b-2 md:  border-r-2 md:border-b-0">
          <div class="flex justify-center">
            <div class="">
              <div
                class="
                  border
                  text-center
                  h-36
                  w-36
                  flex
                  items-center
                  justify-center
                "
              >
                <div>
                  <p class="text-lg text-white">Hours</p>
                  <p class="text-xl text-white font-bold sm:text-2xl">16</p>
                </div>
              </div>

              <div
                class="
                border
                  bg-white
                  text-gray-900 text-center
                  h-36
                  w-36
                  flex
                  items-center
                  justify-center
                "
              >
                <div>
                  <p class="text-lg ">Mins</p>
                  <p class="text-xl sm:text-2xl font-bold">39</p>
                </div>
              </div>

              <div
                class="
                border
                  bg-indigo-500
                  text-center
                  h-36
                  w-36
                  flex
                  items-center
                  justify-center
                "
              >
                <div>
                  <p class="text-lg ">15</p>
                  <p class="text-xl sm:text-2xl font-bold">Secs</p>
                </div>
              </div>
            </div>
            <div
              class="
              border-t border-l border-b
                bg-gray-800
                text-center
                h-84
                w-48
                flex
                items-center
                justify-center
                md:border
              "
            >
              <div>
                <p class="text-xl text-white">Days</p>
                <p class="text-xl font-bold text-white sm:text-3xl">350</p>
              </div>
            </div>
          </div>
        </div>

        <div class="text-left my-6 md:my-0">
          <div class="text-white">
            <svg
              height="28px"
              class="mb-2"
              fill="#667eea"
              version="1.1"
              id="Capa_1"
              xmlns="http://www.w3.org/2000/svg"
              xmlns:xlink="http://www.w3.org/1999/xlink"
              x="0px"
              y="0px"
              viewBox="0 0 32 32"
              xml:space="preserve"
            >
              <g>
                <g id="right_x5F_quote">
                  <g>
                    <path
                      d="M0,4v12h8c0,4.41-3.586,8-8,8v4c6.617,0,12-5.383,12-12V4H0z"
                    />
                    <path
                      d="M20,4v12h8c0,4.41-3.586,8-8,8v4c6.617,0,12-5.383,12-12V4H20z"
                    />
                  </g>
                </g>
              </g>
            </svg>
            <p class="mt-2 text-base leading-6 font md:text-lg">
              Websync akan segera menyediakan template premium untuk anda dengan
              harga yang murah , responsive dan gampang di edit oleh anda .
            </p>
            <div class="text-sm mt-5">
              <a
                href="#"
                class="
                  text-base
                  font-medium
                  leading-none
                  text-indigo-500
                  hover:text-white
                  transition
                  duration-500
                  ease-in-out
                "
                >RK</a
              >
              <p class="text-base md:text-lg">CEO</p>
            </div>
          </div>
        </div>
      </div>
    </div>
  </div>
</template>

<script>
export default {};
</script>

<style scoped>
@import url("https://fonts.googleapis.com/css2?family=Poppins:wght@300;400;500;600;700&display=swap");

.font {
  font-family: "Poppins", sans-serif !important;
}

.template-background {
  background-color: rgb(55 65 81);
  height: 100vh;
  width: 100%;
}

hr {
  margin: auto;
  width: 40%;
}
</style>